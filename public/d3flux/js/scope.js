...
window.onInitFlux = function (cb) {
    window.fluxMain = function (data) {
        require(["cola", "d3", "math"], function (cola, d3, math) {
            function main (model) {
                ...
                if (true) {
                    var svg = d3.select("#d3flux001").append("svg")
                        .attr("viewBox", "0 0 900 620")
                        .attr("style", "display:block;margin:auto;width:100%;height:100%");
                } else {
                ...
                svg.selectAll(".link")
                    .attr("stroke-width", function (d) {
                        return get_flux_width(d.rxn);
                    })
                    .attr("stroke", function (d) {
                        return get_flux_stroke(d.rxn);
                    })
                    ...
                    .on("click", function () {
                        ...
                        force.nodes().forEach(function (node) {
                            if (node.fixed) {
                                if (node.type == "rxn") {
                                    // Reaction object
                                    obj = $.grep(model.reactions, function (e) {
                                        return e.id == node.id;
                                    })[0];
                                    if (!("notes" in obj)) {
                                        obj.notes = {};
                                    }
                                    if (!("map_info" in obj.notes)) {
                                        obj.notes.map_info = {};
                                    }
                                    ...
                                    // Look in metabolites
                                    if (!('cofactor' in node)) {
                                        obj = $.grep(model.metabolites, function (e) {
                                            return e.id == node.id;
                                        })[0];
                                        if (!("notes" in obj)) {
                                            obj.notes = {};
